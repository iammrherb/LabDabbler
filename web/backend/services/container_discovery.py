...
class ContainerDiscoveryService:
    ...
    async def discover_all_containers(self) -> Dict:
        ...
        containers = {
            "containerlab_kinds": await self.discover_containerlab_kinds(),
    ...
    async def discover_containerlab_kinds(self) -> List[Dict]:
        """Discover all containerlab kinds with comprehensive vendor coverage"""
        logger.info("Discovering containerlab kinds...")
        
        containerlab_kinds = []
        
        # Nokia Devices
        nokia_kinds = await self._get_nokia_containerlab_kinds()
        containerlab_kinds.extend(nokia_kinds)
        
        # Arista Devices  
        arista_kinds = await self._get_arista_containerlab_kinds()
        containerlab_kinds.extend(arista_kinds)
        
        # Juniper Devices
        juniper_kinds = await self._get_juniper_containerlab_kinds()
        containerlab_kinds.extend(juniper_kinds)
        
        # Cisco Devices
        cisco_kinds = await self._get_cisco_containerlab_kinds()
        containerlab_kinds.extend(cisco_kinds)
        
        # Other Major Vendors
        other_kinds = await self._get_other_vendor_containerlab_kinds()
        containerlab_kinds.extend(other_kinds)
        
        # Security Vendor Devices
        security_kinds = await self._get_security_containerlab_kinds()
        containerlab_kinds.extend(security_kinds)
        
        # Open Source and Specialized Devices
        opensource_kinds = await self._get_opensource_containerlab_kinds()
        containerlab_kinds.extend(opensource_kinds)
        
        # Infrastructure and Generic Kinds
        infra_kinds = await self._get_infrastructure_containerlab_kinds()
        containerlab_kinds.extend(infra_kinds)
        
        logger.info(f"Discovered {len(containerlab_kinds)} containerlab kinds")
        return containerlab_kinds
    
    async def _get_nokia_containerlab_kinds(self) -> List[Dict]:
        """Get all Nokia containerlab kinds"""
        return [
            {
                "name": "Nokia SR Linux",
                ...
                "config_template": {
                    ...
                    "syscalls": ["sys_admin", "sys_ptrace"],
                    "capabilities": ["SYS_ADMIN", "NET_ADMIN"]
                },
                "supported_versions": ["latest", "24.10.1", "24.7.2", "24.3.3"],
    ...
    async def _get_arista_containerlab_kinds(self) -> List[Dict]:
        """Get all Arista containerlab kinds"""
        return [
            {
                "name": "Arista cEOS",
                "image": "ceos:latest",
                ...
                "config_template": {
                    "env": {"CEOS": "1", "EOS_PLATFORM": "ceossim", "container": "docker"},
                    "syscalls": ["sys_admin"],
                    "capabilities": ["SYS_ADMIN", "NET_ADMIN", "SYS_NICE", "NET_BROADCAST", "SYS_PTRACE"]
                },
                "supported_versions": ["latest", "4.32.0F", "4.31.2F", "4.30.5M"],
                "default_credentials": {"username": "admin", "password": "admin"}
            },
            {
                "name": "Arista vEOS",
    ...
    async def _get_juniper_containerlab_kinds(self) -> List[Dict]:
        """Get all Juniper containerlab kinds"""
        return [
            {
                "name": "Juniper cRPD",
                "image": "crpd:latest",
    ...
    async def _get_cisco_containerlab_kinds(self) -> List[Dict]:
        """Get all Cisco containerlab kinds"""
        return [
            {
                "name": "Cisco XRd",
    ...
    async def _get_other_vendor_containerlab_kinds(self) -> List[Dict]:
        """Get other major network vendor containerlab kinds"""
        return [
            {
                "name": "Cumulus VX",
    ...
    async def _get_security_containerlab_kinds(self) -> List[Dict]:
        """Get security vendor containerlab kinds"""
        return [
            {
    ...
    async def _get_opensource_containerlab_kinds(self) -> List[Dict]:
        """Get open source and specialized containerlab kinds"""
        return [
            {
                "name": "FD.io VPP",
    ...
    async def _get_infrastructure_containerlab_kinds(self) -> List[Dict]:
        """Get infrastructure and generic containerlab kinds"""
        return [
            {
